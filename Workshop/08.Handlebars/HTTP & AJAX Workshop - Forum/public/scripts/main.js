  ...
  function loadMessagesContent(data) {
    ...
    if (data.result.messages && data.result.messages.length > 0) {
      data.result.messages.forEach((msg) => {
        let postDate = msg.postDate.substring(5, 16).replace('T', '/');
        messagesContainer.append(getMsgUI(msg.content, msg.username, postDate));
      });
    } else {
  ...
  contentContainer.on('click', '#btn-add-thread', (ev) => {
    ...
    data.threads.add(title)
      .then(thread => {
        ...
        let postDate = thread.postDate.substring(5, 16).replace('T', '/');
        let element = getThreadUI(thread.title, thread.id, thread.user.username, postDate);

        $('#input-add-thread').val('');
        element.insertBefore($('.navbar-form.navbar-left'));
      })
